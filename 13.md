...
Create `PolicyController.java`, remember this is not a best practices course of REST APIs, so I will not spend time in DTOs on top of Commands and some best practices on error handling :)
```java
...
public class PolicyController {
    ...
    public ResponseEntity<BaseResponse> cancelPolicy(@PathVariable String id) {
        ...
        try {
            ...
            logger.log(Level.SEVERE, errorMessage,e);
            return new ResponseEntity<>(new CancelPolicyResponse(errorMessage), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
